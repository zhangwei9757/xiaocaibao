...
//		}
		...
		GameServer.getInstance().pushStaResource_log(this.id, hero, 0, 1, reason);
		return hb;
	}
	...
	public List<Integer> payHero(int hero, int count, String reason) {
		...
		GameServer.getInstance().pushStaResource_log(this.id, hero, 0, -count, reason);
		return rtn;
	}
...
//		}
		...
		GameServer.getInstance().pushStaResource_log(this.id, equip, 0, 1, reason);
		return hb;
	}
	...
	public void payEquip(int eid, String reason) {
		...
		GameServer.getInstance().pushStaResource_log(this.id, eid, 0, 1, reason);
		equips.remove(eid);
	}
	...
	public EquipBean payTreasure(int key, String reason) {
		...
		GameServer.getInstance().pushStaResource_log(this.id, key, 0, 1, reason);
	...
	public synchronized void addItem(int key, long count, String reason) {
		...
		switch (key) {
			...
			GameServer.getInstance().pushStaResource_log(this.id, key, coin, count, reason);
			return;
			case 钻石: {
				StaBean sb = DaoGame.getInstance().findSta(this.id);
				sb.fixGem(count);
			...
			GameServer.getInstance().pushStaResource_log(this.id, key, gem, count, reason);
			return;
		}
		// 增加碎片，就修改反向索引
		...
		int left = this.items.getOrDefault(key, 0);
		GameServer.getInstance().pushStaResource_log(this.id, key, left, count, reason);
	}
	...
	public synchronized long payItem(int key, long val, String reason) {
		...
		switch (key) {
			...
			int left = this.items.getOrDefault(key, 0);
			GameServer.getInstance().pushStaResource_log(this.id, key, left, -val, reason);
			return val;
			case 钻石: {
				gem -= (int) val;
				if (gem < 0) {
			...
			int left2 = this.items.getOrDefault(key, 0);
			GameServer.getInstance().pushStaResource_log(this.id, key, left2, -val, reason);
			return val;
		}
		...
		if (has == val) {
			items.remove(key);
			GameServer.getInstance().pushStaResource_log(this.id, key, 0, -val, reason);
			return val;
		}
		else if (has < val) {
