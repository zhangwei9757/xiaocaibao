...
public class TreasureRankService {
	static final Log log = LogFactory.getLog(TreasureRankService.class);
	...
	void init() {
		_instance = this;
		log.warn("读取 宝藏排名数据，启动的时候从数据库中读取，每隔5分钟进行一次保存");
		...
		log.warn("结束读取 宝藏排名数据，启动的时候从数据库中读取，每隔5分钟进行一次保存");
	}
			...
			synchronized (this)
			{
				...
				for (DtrankConf dc : dcs) {
					...
					for (int i = begin; i <= dc.key && i <= max; ++i) {
						...
						"神秘宝藏积分排名中位于[" + (trb.getRank() + 1) + "]名，特发送以下奖励!");
