...
import com.tumei.controller.struct.GroupPreRole;
import com.tumei.controller.struct.GroupRole;
import com.tumei.controller.struct.GroupScene;
...
public class GroupBean {
     ...
     * 暂存的 需要广播的聊天消息
     */
    private List<MessageDto> chats = new ArrayList<>();
    ...
    public synchronized void update() {
        ...
        if (chats.size() > 0) {
            try {
                zoneRoles.forEach((server, grs) -> {
                    List<Long> rs = grs.stream().mapToLong((gr) -> gr.id).boxed().collect(Collectors.toList());
                    RemoteService.getInstance().sendGuildMessage(server, rs, chats);
                });
            } catch (Exception ex) {
    ...
    public synchronized String join(GroupRoleMessage grm) {
        ...
        if (approval == 2 && roles.size() > 0) {
            ...
            if (roles.values().stream().anyMatch((rr) -> (rr.id == grm.id))) {
                return "已经加入该公会";
            }

            long other = GroupService.getInstance().tryJoinGroup(grm.id, id);
            if (other != 0) {
                return "other:" + other;
            }
    ...
    public synchronized String approveJoin(GroupPreRole gpr, GroupRole leader) {
        ...
        long other = GroupService.getInstance().tryJoinGroup(gpr.id, id);
        if (other != 0) {
            return "玩家已经加入其它公会";
        }
    ...
    public synchronized String leave(long id) {
        ...
        if (role != null) {
            if (roles.size() > 1) {
                ...
                if (role.isLord()) {
                    Optional<GroupRole> opt = roles.values().stream().filter(GroupRole::isVp).findFirst();
    ...
    public synchronized GroupMessage createBody() {
        ...
        getRoles().values().forEach((role) -> gm.roles.add(role.createGroupRoleMessage()));
        getPreRoles().forEach((role) -> gm.pres.add(role.createGroupRoleMessage()));
    ...
    public synchronized GroupSimpleStruct createSimpleBody() {
        ...
        Optional<GroupRole> opt = getRoles().values().stream().filter(GroupRole::isLord).findFirst();
    ...
    public synchronized String getLeaderName() {
        Optional<GroupRole> opt = roles.values().stream().filter(GroupRole::isLord).findFirst();
    ...
    public synchronized void sendChat(MessageDto dto) {
        chats.add(dto);
    }
     ...
     * @return
     */
    public synchronized int donate(long role, int pg, int _exp, int cb) {
        ...
        if (gbb != null) {
            gbb.flush();
            rtn = gbb.buildBag(flag, this.getId());
        }
     ...
     *
     */
    public synchronized void validatePayment(int mode, String name) {
        if (gbb.flush()) {
            dirty = true;
        }

        if (gbb.open(mode, name, this.id)) {
            dirty = true;
        }
    }
     ...
     * @return
     */
    public synchronized List<GuildbagBasicDto> getBagInfos(long role) {
        List<GuildbagBasicDto> dto = new ArrayList<>();
        if (gbb.flush()) {
            dirty = true;
        }
     ...
     * @return
     */
    public synchronized GuildbagDetailDto getBagDetail(long role, String name, String bag) {
        if (gbb.flush()) {
            dirty = true;
        }

        GuildbagDetailDto gdd = gbb.receive(role, name, bag, this.id);
    ...
    public synchronized void callFight(GroupRole role, HerosStruct bs, int index, BattleResultStruct rl) {
        ...
        if (peers.stream().noneMatch((dhs) -> dhs.life > 0)) {
            ...
            try {
                FightResult r = GroupService.getInstance().getBattle().doSceneBattle(bs, peers, 0, true, 0, 0, 0, 0);
