...
class GroovyMineSystem implements IMineSystem {
    private Log log = LogFactory.getLog(GameServer.class)
...
//                log.error("刷新矿脉 key:" + key + " 非法长度。")
                ...
                if (amb.uid != 0) {
                    ...
                    if (dura >= (amb.needToDefende(msc))) {
                        ...
                        RoleMineBean rmb = DaoService.getInstance().findRoleMap(mid)
                        if (rmb != null) {
                            rmb.setOccupy(-1)
     ...
     * @return
     */
    public boolean checkExistsMine(long id, int mine) {
     ...
     * @param awards
     */
    public void onMineOver(MineBean mine, MineRole robber, long _id, int rebirth, List<Integer> awards) {
     ...
     * @param awards
     */
    public void onMineOver(MineBean mine, long _id, List<Integer> awards) {
        MineRole mr = roles.getOrDefault(_id, null)
        if (mr != null) {
     ...
     * @param mine
     */
    public void notifyMineChange(MineBean mine) {
        // 通知这个矿周围的玩家，矿发生了变化
...
//            e.printlnStackTrace()
     ...
     * @param func
     */
    public void lookMine(HexNode _node, BiConsumer<Boolean, MineBean> func) {
     ...
     * @return
     */
    public synchronized int beatMine(long id, int pos, int level, RequestMineAction.Return rl) {
        ...
        if (amb.uid == 0) { // 空矿
            ...
            if (r.win == 1) {  // 胜利
                ...
                RoleMineBean rmb = DaoService.getInstance().findRoleMap(peer)
                if (rmb != null) {
                    rmb.setOccupy(-1)
                    // 被打败后重置到出生点
                    rmb.reset()
     ...
     * 玩家请求进入地图
     */
    public synchronized String enter(GameUser user, RoleMineBean rmb) {
    ...
    synchronized void accelerate(GameUser user, int pos, int level, RequestMineAccelerate.Return rl) {
        ...
        if (gem > 0) {
            ...
            RoleMineBean rmb = DaoService.getInstance().findRoleMap(user.getUid())
            if (rmb != null) {
                rmb.setOccupy(-1)
                MineAwardBean mab = rmb.getMineAward()
                mab.key = mss.key
     ...
     * @return
     */
    public synchronized int move(long id, int dir, int pos) {
        MineRole mr = roles.getOrDefault(id, null)
        if (mr == null) {
            return -1
        }
     ...
     * @param pos
     */
    public synchronized void look(long id, int pos) {
        MineRole mr = roles.getOrDefault(id, null)
        if (mr == null) {
            return
        }
...
//                println("抢劫矿脉有:" + k + "," + v)
     ...
     * @return
     */
    public void removeTask(long id, int pos) {
        MineRole mr = roles.getOrDefault(id, null)
        if (mr != null) {
     ...
     * @return
     */
    public int hasTaskInDirection(long id, int dir) {
        MineRole mr = roles.getOrDefault(id, null)
        if (mr == null) {
            return -1
        }
     ...
     * 2. 当前结点添加玩家
     */
    public void fixRoleInNode(MineRole mr, HexNode last, HexNode lastLook) {
     ...
     * @param last
     */
    public void fixRoleInNodeLook(MineRole mr, HexNode last) {
        long id = mr.user.getUid()
     ...
     */
    public void onMove(MineRole mr, int _dir) {
        // 1. 计算当前的周围玩家
...
//                    println(id + "的视野中增加玩家:" + key + " grade:" + _mr.grade)
     ...
     * @param mr
     */
    public void onLookMove(MineRole mr) {
        // 1. 计算当前的周围玩家
...
//        }
     ...
     * @return
     */
    public HexNode getNode(int position) {
    ...
    public int distance(HexNode hn, int b) {
        HexNode hm = getNode(b)
        if (hm == null) {
            return 10000
        }
    ...
    public HexNode neighbour(HexNode node, int dir) {
        int pos = node.hex.neighbour(dir - 1).ID()
        return nodes.getOrDefault(pos, null)
    }

    public HexNode neighbour(int position, int dir) {
     ...
     * @return
     */
    public List<HexNode> neighbours(HexNode node, int distance) {
     ...
     */
    @Override
    public synchronized String dumpPlayers() {
        String msg = ""
        roles.forEach({ k, v ->
    ...
    @Override
    public void enter(GameUser user, RequestMineEnter proto) {
    ...
    @Override
    public void leave(GameUser user, RequestMineLeave proto) {
    ...
    @Override
    public void move(GameUser user, RequestMineMove proto) {
    ...
    @Override
    public void look(GameUser user, RequestMineLook proto) {
     ...
     * @param proto
     */
    public void enhance(GameUser user, RequestMineEnhance proto) {
     ...
     * @param proto
     */
    public void buyEnergy(GameUser user, RequestMineBuyEnergy proto) {
...
//		}
     ...
     * @param proto
     */
    public void harvest(GameUser user, RequestMineHarvest proto) {
     ...
     * @param proto
     */
    public void accelerate(GameUser user, RequestMineAccelerate proto) {
     ...
     * @param proto
     */
    public void action(GameUser user, RequestMineAction proto) {
                    ...
                    case 2: // 怪物
                        ...
                        if (pos < 0) {
                            ...
                            if (mts == null) {
                                ...
                                MineMonsterConf mmc = Readonly.getInstance().getMineMonsterConfs().get(mts.level - 1)
                                // 战斗
...
class MineRole {
    ...
    public MineRole(GroovyMineSystem _service, GameUser _user) {
        service = _service
        user = _user
    }
...
//        GroupBean gb = DaoService.getInstance().findGroup(id)
     ...
     * @param _dir
     */
    public void move(HexNode _node, int _dir) {
        HexNode last = node
     ...
     */
    public void lookMove(HexNode _node) {
    ...
    public boolean removeStaticSeeList(int pos) {
        sSeeList.remove(pos)
        return true
    }
    ...
    private void lookStatic(RoleMineBean rmb) {
        ...
        if (rmb == null) {
            rmb = DaoService.getInstance().findRoleMap(user.getUid())
        }
     ...
     * @param _behave
     */
    public void addPsNotify(long _id, int _behave, int _pos) {
        synchronized (infos) {
     ...
     * @param _awards
     */
    public void addMineNotify(int _key, String _robber, int _grade, int _rebirth, List<Integer> _awards) {
     ...
     * @param _awards
     */
    public void addMineNotify(int _key, List<Integer> _awards) {
        NotifyMineFinish nmf = new NotifyMineFinish()
        nmf.key = _key
        nmf.awards = _awards
     ...
     * 玩家占领矿
     */
    public int occupy(MineBean mss) {
        long now = System.currentTimeMillis() / 1000
