			...
			for (MineRefreshConf mrc : mrcs) {
				...
				switch (mrc.type) {
					...
					case 2: // 宝箱
					{
						...
						md.treasure.stream().filter((ho) -> ho.level == mrc.level).mapToInt((ho) -> ho.pos).forEach((p) -> positions.add(p));
						// 随机这个列表
					...
					{
						List<Integer> positions = new ArrayList<>();
						md.monster.stream().filter((ho) -> ho.level == mrc.level).mapToInt((ho) -> ho.pos).forEach((p) -> positions.add(p));
						// 随机这个列表
						Collections.shuffle(positions);
						// 取出前N个
						int max = Math.min(mrc.num, positions.size());
						for (int i = 0; i < max; ++i) {
							int pos = positions.get(i);
							MineMonsterStruct mts = new MineMonsterStruct(pos, mrc.level);
							monster.put(pos, mts);
						}
					}
					break;
					...
					{
						List<Integer> positions = new ArrayList<>();
						md.npc.stream().filter((ho) -> ho.level == mrc.level).mapToInt((ho) -> ho.pos).forEach((p) -> positions.add(p));
						// 随机这个列表
						Collections.shuffle(positions);
						// 取出前N个
						int max = Math.min(mrc.num, positions.size());
