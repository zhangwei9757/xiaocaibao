...
public class SceneBean {
	...
	public void harvest(GameUser user) {
		...
		if (rb != null) {
			while (cumerate >= rb.cd) {
				...
				cumerate -= rb.cd;
				int gold = (int)(rb.gold / 3600f * rb.cd);
				int exp = (int)(rb.exp / 3600f * rb.cd);
				...
				if (!trigger) {
					user.triggerSceneEvent(rb, (int)(rb.gold / 3600f * rb.cd), (int)(rb.exp / 3600f * rb.cd));
				}
			}
		}
	}
	...
	public void harvest(GameUser user, OfflineAwardStruct bean, long extra) {
		...
		if (rb != null) {
			while (cumerate >= rb.cd) {
				...
				// 节日活动期间可以有新的东西出现
				FestivalBean fb = DaoService.getInstance().findFestival(this.id);
	...
	public void harvest(GameUser user, int diff, RequestSceneFast.ReturnSceneFast sf) {
		...
		if (rb != null) {
			...
			if (gold > 0) {
				PackBean pb = user.getDao().findPack(user.getUid());
				pb.addItem(金币, gold, "快速战斗");
				sf.awards.add(new AwardBean(金币, gold, 0));
			}
			if (exp > 0) {
				user.addLordExp(exp);
