...
public class GroupService {
    ...
    public List<GuildbagBean> guildbagBeans = new ArrayList<>();
    ...
    public synchronized void addGroup(GroupBean gb) {
        ...
        int zone = serviceRouter.chooseZone(gb.getId());
...
//						log.info("保存公会:" + gb.getId());
    ...
    public synchronized GroupBean create(long gid, String name) {
        ...
        int zone = serviceRouter.chooseZone(gid);
        GroupBean gb = GroupBean.create(gid, zone);
        gb.setName(name);
     ...
     * @param rs
     */
    public synchronized boolean fillRankStruct(long gid, RankStruct rs) {
        GroupBean gb = groups.get(gid);
        if (gb != null && !Strings.isNullOrEmpty(gb.getName())) {
            if (gb.getName().equals("null")) {
                return false;
            }
...
//				return gss;
    ...
    public GuildbagBean findGuildbags(int id) {
        try {
            return guildbagBeans.get(id);
        } catch (Exception e) {
