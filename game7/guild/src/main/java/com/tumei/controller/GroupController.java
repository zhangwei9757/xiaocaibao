...
public class GroupController {
    ...
    @ApiOperation(value = "捐献成功，随机生成红包")
    ...
    public void DonateSuccess(long gid, long role, int mode) {
        try {
            GroupBean gb = groupService.find(gid);
            if (gb == null) {
                return;
    ...
    public
    @ResponseBody
    Integer PaymentValidation(long gid, long role) {
        try {
            GroupBean gb = groupService.find(gid);
            if (gb == null) {
                return -1;
    ...
    public
    @ResponseBody
    Integer GuildBagList(long gid, long role) {
        try {
            GroupBean gb = groupService.find(gid);
            if (gb == null) {
                return -1;
    ...
    public
    @ResponseBody
    Integer GuildBagReceive(long gid, long role) {
        try {
            GroupBean gb = groupService.find(gid);
            if (gb == null) {
                return -1;
