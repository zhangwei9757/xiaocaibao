 ...
 */
@ConditionalOnProperty(value = "xcb.cache.enabled", havingValue = "true")
@Component
public class GameCache {
    private static Log log = LogFactory.getLog(GameCache.class);

    private List<LoadingCache> caches;

    @Value("${xcb.cache.accDelay:10}")
    private int accDelay;

    @Value("${xcb.cache.writeDelay:10}")
    ...
    public void invalidate(Long uid) {
        ...
        try {
            ...
            log.error("刷新角色到磁盘失败:" + ex.getMessage());
        }
    }
