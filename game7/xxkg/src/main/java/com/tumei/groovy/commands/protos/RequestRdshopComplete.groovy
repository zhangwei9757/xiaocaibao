...
class RequestRdshopComplete extends BaseProtocol {
    ...
    void onProcess(WebSocketUser session) {
        ...
        RdshopConf rsc = Readonly.instance.getRdshop().get(rb.rs.key)
        if (rb.rs != null) {
            // 事件超时，直接进入下一个事件
            if (System.currentTimeMillis() / 1000 >= rb.rs.complete) {
                rb.rs = null
                r.rs = rb.flush(user)
                r.count = rb.count
                r.result = "当前事件已过期"
            ...
            if (rsc.type == 1) {
                ...
                HerosBean hsb = DaoGame.instance.findHeros(user.getUid())
