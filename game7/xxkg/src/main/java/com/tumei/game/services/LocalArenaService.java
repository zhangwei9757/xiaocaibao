 ...
 */
@Service
public class LocalArenaService {
	private final static Log log = LogFactory.getLog(LocalArenaService.class);

	private static LocalArenaService _instance;

	public static LocalArenaService getInstance() {
		return _instance;
	}
	...
	private final Object all_rank_monitor = new Object();
				...
				if (rk.size() == 0) { // Role.Ranks 为空的时候，需要创建1500个机器人
					...
					for (RobotConf rc : rcs) {
						for (int i = start; i < rc.rank; ++i) {
							...
							hsb.setHeros(hbs);
							long power = GameServer.getInstance().getBattleSystem().calc_power(hsb.createHerosStruct());
							rb.setPower(power);
							rk.add(rb);
	...
	void arenaSchedule() {
		...
		synchronized (this) {
			ranks.forEach( r -> rk.add(r.getId()) );
		}
		...
		for (int i = 0; i < rk.size(); ++i) {
			...
			if (ac != null) {
				...
				for (int rr : ac.rankreward) {
					awards.append(rr).append(",");
				}

				GameServer.getInstance().sendAwardMail(rk.get(i), "竞技场排名奖励", "竞技场排名" + (i+1), awards.toString());
			}
		}
	}
	 ...
	 * @return
	 *
	 * 如果玩家还没有竞技场排名,标识第一次进入,直接追加在最后
	 *
	 */
	public synchronized RankBean getRank(long id) {
		boolean _flush = false;
		RankBean rb = users.getOrDefault(id, null);
		if (rb == null) {
	...
	public synchronized int exchange(long uid, long id, int rank) {
		...
		if (rank < selfRank) {
			...
			// 1500名以内,每提升1个名次给与1个钻石的奖励
			if (rank < 1500) {
				int peek = self.getPeek();
				if (peek > 1500) {// 历史排名不到1500,归入到1500,在和本次的排名比较,提升多少,奖励多少钻石
					peek = 1500;
				}
