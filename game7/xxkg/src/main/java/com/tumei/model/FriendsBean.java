...
//		}
	...
	public void notifyAllFriends(long power, RoleBean rb) {
		...
		for (long fid : fds) {
			FriendsBean fsb = DaoGame.getInstance().findFriends(fid);
	...
	public String applyFriend(long _id, long _power) {
		...
		if (!DaoGame.getInstance().findFriends(_id).addApplyFriend(fb)) {
			return "对方好友已达上限";
		}
	...
	public String confirmFriend(long _id, long _power, int _flag) {
		...
		if (DaoGame.getInstance().findFriends(_id).notifyAddFriend(my) == 0) {
			// 删除
	...
	public void removeFriend(long _id, boolean _relevant) {
		...
		if (_relevant && fb != null) {
			FriendsBean fsb = DaoGame.getInstance().findFriends(_id);
