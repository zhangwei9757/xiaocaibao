...
public class Readonly extends IReadonly {
    ...
    class Conf {
        ...
        public List<SummonrankConf> summonrankConfs = new ArrayList<>();
        public List<VigorrankConf> vigorrankConfs = new ArrayList<>();
        ...
        public void initialize() {
            ...
            summonrankConfs = mongoTemplate.findAll(SummonrankConf.class);
            summonrankConfs.sort((o1, o2) -> {
                if (o1.key < o2.key) {
                    return -1;
                } else if (o1.key > o2.key) {
                    return 1;
            ...
            vigorrankConfs = mongoTemplate.findAll(VigorrankConf.class);
            vigorrankConfs.sort((o1, o2) -> {
                if (o1.key < o2.key) {
                    return -1;
                } else if (o1.key > o2.key) {
                    return 1;
    ...
    class FestConf {
        ...
        public void initialize() {
            ...
            this.crazylistConfs = mongoTemplate.findAll(CrazylistConf.class);
            crazylistConfs.sort((o1, o2) -> {
                if (o1.key < o2.key) {
                    return -1;
                } else if (o1.key > o2.key) {
                    return 1;
     ...
     * 英雄狂欢
     */
    public SummonrankConf findSummonrank(int key) {
        --key;
        if (key >= conf.summonrankConfs.size()) {
            return null;
        }
        return conf.summonrankConfs.get(key);
    }

    public List<SummonrankConf> getSummonranks() {
        return conf.summonrankConfs;
    }

    /**
     * 活力狂欢
     */
    public VigorrankConf findVigorrank(int key) {
        --key;
        if (key >= conf.vigorrankConfs.size()) {
            return null;
        }
        return conf.vigorrankConfs.get(key);
    }

    public List<VigorrankConf> getVigorranks() {
        return conf.vigorrankConfs;
    }
    ...
    public List<CrazylistConf> getCrazylistConfs() {
        return festConf.crazylistConfs;
    }

    public CrazylistConf findCrazylistConf(int key) {
        if (key == -1) {
            return null;
        }
        return festConf.crazylistConfs.get(key - 1);
    }
    ...
    public FestivalConf findFestivalConfByKey(int key) {
        if (key <= 0) {
            return null;
        }
        return festConf.festivalConfs.stream().filter(fc -> fc.mode == key).findFirst().get();
    }
